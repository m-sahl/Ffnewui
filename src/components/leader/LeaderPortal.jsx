        ...
        <div style={{ paddingTop: 20, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 0, borderRadius: 12, overflow: "hidden", background: dark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", padding: 4, marginBottom: 14 }}>
            {[
              ...
              { id: "events",  label: "Registrations" },
            ].map(t => (
              <button key={t.id} onClick={() => { setTab(t.id); setCatFilter("All"); }}
                style={{
          ...
          {/* Stage / Off-Stage toggle — only on events tab */}
          {tab === "events" && (
            <div style={{ display: "flex", gap: 0, borderRadius: 12, overflow: "hidden", background: dark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", padding: 4, marginBottom: 14 }}>
                  ...
                  <tbody>
                    {filtStudents.map(s => (
                      <tr key={s.id}>
                        ...
                        <td><span style={{ fontSize: 12, color: mutedTx, fontWeight: 500 }}>{s.groupRole || "Member"}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="anim-fadeIn">
            {filtRegs.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 0", color: mutedTx }}>
                ...
                <div style={{ fontSize: 13 }}>No {session} events registered{catFilter !== "All" ? ` in ${catFilter}` : ""}</div>
              </div>
            ) : (
                  ...
                  return (
                    <div key={r.id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: parts.length ? 8 : 0 }}>
      ...
      {regModal && (
        <Modal title={editTarget ? "Edit Registration" : "Register for Event"} onClose={() => setRegModal(false)} wide>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <label className="label">Program ({session})</label>
        ...
        <div className="modal-bg" onClick={() => setDelConfirm(null)}>
          <div className="modal" style={{ maxWidth: 320, textAlign: "center" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button className="btn" style={{ flex: 1, height: 44, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.07)", color: dark ? "#e8e8f5" : "#12121e", fontWeight: 700 }} onClick={confirmDelete}>Remove</button>
            </div>
          </div>
        </div>
      )}
