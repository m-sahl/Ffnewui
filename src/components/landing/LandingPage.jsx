      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420 }}>
        ...
        <div className="anim-fadeUp" style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ position: "relative", width: 88, height: 88, margin: "0 auto 20px" }}>
            <div style={{ position: "absolute", inset: -12, borderRadius: "50%", border: "1px solid rgba(245,158,11,0.2)", animation: "orbitSpin 8s linear infinite" }}>
            ...
            <div style={{ width: 88, height: 88, borderRadius: 26, background: "linear-gradient(145deg, #f59e0b, #d97706)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 12px 40px rgba(245,158,11,0.4)", animation: "glowPulse 3s infinite", fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: 36, color: "#0a0b12", letterSpacing: -1 }}>FF</div>
          </div>
          <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: 38, letterSpacing: -1.5, lineHeight: 1, background: "linear-gradient(135deg, #fbbf24, #f59e0b, #fef3c7, #f59e0b)", backgroundSize: "200% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", animation: "goldShimmer 4s linear infinite", marginBottom: 6 }}>FestFlow</h1>
          <p style={{ fontSize: 13, color: dark ? "#4b5563" : "#9ca3af", fontWeight: 500 }}>Arts & Cultural Fest</p>
        </div>
          ...
          {groups.map((group, i) => {
            ...
            return (
              <button key={group.id} onClick={() => setLoginGroup(group)}
                ...
                style={{ animationDelay: `${0.06 * i}s`, display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", background: dark ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.85)", border: `1px solid ${dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)"}`, borderRadius: 16, cursor: "pointer", width: "100%", fontFamily: "inherit", transition: "all 0.2s cubic-bezier(0.22,1,0.36,1)" }}
              ...
              >
                <div style={{ width: 42, height: 42, borderRadius: 13, flexShrink: 0, background: `${color}18`, color, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: 18 }}>
                  {group.name.charAt(0).toUpperCase()}
                </div>
                <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: 15, color: dark ? "#e8e8f5" : "#12121e", flex: 1, textAlign: "left" }}>{group.name}</span>
                <div style={{ color: dark ? "#2d2f3a" : "#d1d5db", flexShrink: 0 }}><Ic name="chevronRight" size={16} /></div>
              </button>
            );
          })}
        </div>
