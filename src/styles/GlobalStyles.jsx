...
const GlobalStyles = ({ dark }) => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes glowPulse     { 0%,100%{box-shadow:0 0 24px rgba(245,158,11,0.3);} 50%{box-shadow:0 0 48px rgba(245,158,11,0.65);} }
    ...
    .btn-primary {
      ...
      color: #0a0b12; font-weight: 700;
      box-shadow: 0 4px 18px rgba(245,158,11,0.38);
    }
    .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(245,158,11,0.52); }

    .btn-ghost {
      background: ${dark ? "rgba(255,255,255,0.055)" : "rgba(0,0,0,0.045)"};
      color: ${dark ? "#9ca3af" : "#6b7280"};
    }
    .btn-ghost:hover { background: rgba(245,158,11,0.1); color: #f59e0b; }

    .btn-danger { background: rgba(225,29,72,0.1); color: #e11d48; border: 1px solid rgba(225,29,72,0.2); }
    .btn-danger:hover { background: rgba(225,29,72,0.18); transform: translateY(-1px); }
    ...
    .input:focus { border-color: #f59e0b; box-shadow: 0 0 0 3px rgba(245,158,11,0.14); background: ${dark ? "rgba(245,158,11,0.04)" : "rgba(245,158,11,0.03)"}; }
    .input::placeholder { color: ${dark ? "#4b5563" : "#9ca3af"}; }
    .input-icon-wrap { position: relative; }
    .input-icon-wrap .input { padding-left: 40px; }
    .input-icon { position: absolute; left: 13px; top: 50%; transform: translateY(-50%); color: ${dark ? "#4b5563" : "#9ca3af"}; pointer-events: none; }

    .label { display: block; font-size: 11.5px; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; color: ${dark ? "#6b7280" : "#9ca3af"}; margin-bottom: 7px; }
    ...
    .tbl tbody tr:hover { background: ${dark ? "rgba(245,158,11,0.055)" : "rgba(245,158,11,0.04)"}; }
    ...
    .badge-junior  { background: rgba(14,165,233,0.14); color: #0ea5e9; }
    .badge-sj      { background: rgba(225,29,72,0.14);  color: #e11d48; }
    .badge-single  { background: rgba(16,185,129,0.14); color: #10b981; }
    ...
    .modal {
      ...
      border: 1px solid ${dark ? "rgba(245,158,11,0.18)" : "rgba(245,158,11,0.15)"};
      border-radius: 22px; padding: 24px; width: 100%; max-width: 480px;
      animation: modalIn 0.28s cubic-bezier(0.34,1.56,0.64,1);
      box-shadow: 0 28px 80px rgba(0,0,0,0.45), 0 0 0 1px rgba(245,158,11,0.08);
    ...
    .pin-dot.filled { background: #f59e0b; border-color: #f59e0b; box-shadow: 0 0 12px rgba(245,158,11,0.55); transform: scale(1.12); }
    ...
    .topbar {
      ...
      background: ${dark ? "rgba(8,9,18,0.92)" : "rgba(245,245,251,0.92)"};
    ...
    .sidebar {
      ...
      padding: 16px 10px; background: ${dark ? "rgba(8,9,18,0.7)" : "rgba(248,248,254,0.8)"};
    ...
    .sidebar-item:hover { background: ${dark ? "rgba(245,158,11,0.07)" : "rgba(245,158,11,0.06)"}; color: #f59e0b; }
    .sidebar-item.active {
      background: rgba(245,158,11,0.12); color: #f59e0b;
      border-color: rgba(245,158,11,0.2);
      box-shadow: inset 0 0 0 1px rgba(245,158,11,0.08);
    }
    .sidebar-section { font-size: 10px; font-weight: 700; letter-spacing: 1.2px; text-transform: uppercase; color: ${dark ? "#374151" : "#c4c4d4"}; padding: 14px 12px 6px; }

    /* ── Leader bottom tab bar ────────────────────────── */
    .tabbar {
      position: fixed; bottom: 0; left: 0; right: 0; z-index: 90;
      display: flex; align-items: center;
      background: ${dark ? "rgba(8,9,18,0.96)" : "rgba(245,245,251,0.96)"};
    ...
    .tab-item {
      ...
      font-size: 11px; font-weight: 600; color: ${dark ? "#4b5563" : "#9ca3af"};
      transition: all 0.18s ease; border: none; background: none; font-family: 'Inter', sans-serif;
    }
    .tab-item.active { color: #f59e0b; background: rgba(245,158,11,0.1); }
    .tab-item:hover { color: #f59e0b; }
    ...
    .live-dot::after { content: ''; position: absolute; inset: -3px; border-radius: 50%; border: 2px solid #10b981; animation: dotPulse 1.5s infinite; }

    /* ── Gradient text ────────────────────────────────── */
    .grad-text {
      background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 40%, #f59e0b 60%, #d97706 100%);
    ...
    .toast-error   { background: rgba(225,29,72,0.92); color: white; }
    .toast-info    { background: rgba(245,158,11,0.92); color: #0a0b12; }
    ...
    .stat-card:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(245,158,11,0.1); }
    ...
    .section-sub { font-size: 12px; color: ${dark ? "#6b7280" : "#9ca3af"}; margin-top: 2px; font-weight: 500; }
    ...
    .empty-desc { font-size: 13px; color: ${dark ? "#6b7280" : "#9ca3af"}; }
